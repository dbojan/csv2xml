...
rem if you want to replace & with " " add this in front of other replace: 

rem replace('&','&amp;').replace('<','&lt;').replace('>','&gt;').replace("'",'&apos;').replace('"','&quot;')

rem 34 	22 	042 	" 	Quotation mark/Double quote
rem 38 	26 	046 	& 	Ampersand
rem 39 	27 	047 	' 	Apostrophe/Single quote
rem 60 	3C 	074 	< 	Less-than sign
rem 62 	3E 	076 	> 	Greater-than sign
rem source https://www.techonthenet.com/ascii/chart.php
rem https://stackoverflow.com/questions/31575784/how-to-escape-powershell-double-quotes-from-a-bat-file
rem ampersand must go first
     
powershell -Command "(gc -encoding utf8 '%tempfile%') -replace  [char]38,'&amp;'  -replace [char]34,'&quot;'  -replace [char]39,'&apos;'    -replace[char]60,'&lt;'   -replace [char]62,'&gt;'            -replace '%separatortype%' , '%comma%'     -join '%crlf%'    | Set-Content -encoding utf8 '%tempfile%' "
